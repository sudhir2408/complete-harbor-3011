...
public class LeaveDAOImpl implements LeaveDAO {
	...
	public String addLeavetoDB(Leave l, int noOfDays) throws LeaveException {
		String message = "****-----Leave NOT Applied , please apply first----***";
		...
		try (Connection conn = DButil.provideConnection()) {
			...
			if (rs.next() && rs.getInt("Leaves_left") < noOfDays) {
				...
				if (n > 0) {
					message = "***congratulation**Leave Applied **Successfully!---***** ";
					...
					throw new LeaveException("****----Reason Too long,please short it--*****");
				}
